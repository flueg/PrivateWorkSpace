...
use File::Basename;
use File::Basename;
use KankanMonitorUtils qw(RunCmd SendSMS SendSMSEconomically SendEMail IsKankanHolidayToday);
...
sub LoadConfiguration
{
	...
	if (defined $host->{'mobileList'})
	{
		$SMS_RECIPIENTS .= join ',', @{$host->{'mobileList'}};
	}
...
sub IsWeekendToday()
{
	...
	# '==' for numeric comparision, 'eq' for string comparision.
	return 1 if ($wday == 0 or $wday == 6);
	return 0;
}
...
MAIL_BODY
	...
	my ($rc, $output) = SendEMail($MAIL_RECIPIENTS, $CC_LIST, $subject, $body);
	if (! $rc)
	{
	...
	# To save SMS cost, only send SMS when administrators are home (weekend or holiday).
	($rc, $output) = IsKankanHolidayToday();
	return unless (IsWeekendToday or $rc);

	my $message = "Hostname: @{$host->{'hostname'}}, IP: @{$host->{'host'}}, Failed services: $failed_services";
	($rc, $output) = SendSMSEconomically($SMS_RECIPIENTS, $message, 3, 2);
	if (! $rc)
	{
		ERROR("Fail to send SMS");
	}
	DEBUG("$output");
}
